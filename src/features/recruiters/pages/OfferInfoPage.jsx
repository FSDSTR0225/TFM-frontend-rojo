...
export const OfferInfoPage = () => {
  ...
  const handleDownloadCoverLetter = async (offerId, userId) => {
    try {
      ...
      new Error("Error downloading PDF");
      throw error;
    }
  };
  ...
  const isOwnerRecruiter =
    offer?.owner?._id === profile?._id && profile?.role.type === "recruiter";
  ...
  const getCandidates = async () => {
    const data = await getCandidatesByOfferId(
      id,
      localStorage.getItem("token")
    );
  ...
  return (
    <SectionContainer
      classProps={"lg:flex-row flex-col-reverse gap-4 lg:items-start"}
    >
      <OfferInfo
        offer={offer}
            ...
            {lists?.length > 0 ? (
              lists.map((candidato) => {
                ...
                return (
                  <div
                    key={candidato._id}
                    className="bg-neutral-80 p-4 gap-2 rounded-lg shadow-sm border border-neutral-60 flex flex-col"
                  >
                    <div className="flex items-center justify-between ">
                      ...
                      <div className="flex flex-col items-center gap-2 ">
                        <div className="flex flex-col items-center gap-2 ">
                          <div
                            ...
                            onClick={() =>
                              navigate(`/profile/${candidato?.user?._id}`)
                            }
                          >
                            <div>
                      ...
                      <div className="hidden min-w-20 sm:flex flex-wrap gap-2 mt-2 text-xs">
                        <CandidateSkills
                          skills={candidato?.user?.role?.developer?.skills}
                        ...
                        ${fadedColors[candidato.status] || "bg-black/20"} ${
                          ...
                          <div className="flex flex-col md:flex-row lg:flex-col intems-center  gap-1 gap-x-2">
                            ...
                            <div className="flex gap-2">
                              <a
                                className="btn btn-md bg-neutral-90 hover:bg-neutral-60"
                                href={`mailto:${candidato?.user?.email}`}
                              >
                                <PiEnvelope size={20} />
                              </a>
                              <button
              ...
              <p className="text-neutral-50">
                No candidates found for "{activeTab}".
              </p>
            )}
          </div>
        </aside>
