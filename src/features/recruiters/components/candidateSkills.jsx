export const CandidateSkills = ({skills,skillsOffer}) => {
  ...
  const matchingSkills = allSkills.filter((skill) => ofertaSkills.includes(skill));
  const nonMatchingSkills = allSkills.filter((skill) => !ofertaSkills.includes(skill));
  ...
  return (
    <>
     {sortedSkills.length > 0 ? (
            ...
            <span className="text-sm text-neutral-400 italic">Sin habilidades registradas</span>
          )}
    </>
  )
}
