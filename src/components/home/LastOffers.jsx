...
export const LastOffers = () => {
  ...
  const [cols, setCols] = useState(1); // para controlar columnas dinámicamente
  ...
  // Detectar columnas según ancho de ventana (match con las clases Tailwind)
  useLayoutEffect(() => {
    const updateCols = () => {
      const width = window.innerWidth;
      if (width >= 1024) {
        setCols(3); // lg:grid-cols-3
      } else if (width >= 640) {
        setCols(2); // sm:grid-cols-2
      } else {
        setCols(1); // grid-cols-1
      }
    };
  ...
  // Definir cuántas ofertas mostrar según columnas:
  let maxShowOffers = 4;
  if (cols === 3) maxShowOffers = 3; // grid-cols-3 => 3 ofertas
  if (cols === 2) maxShowOffers = 4; // grid-cols-2 => 4 ofertas
  if (cols === 1) maxShowOffers = 4; // grid-cols-1 => 4 ofertas
      ...
      <div>
        ...
        <div className="grid grid-cols-2">
          <span className="text-neutral-20">
            Find your next professional challenge
          </span>
          <Link
            to={`/offers`}
            className="justify-self-end bg-transparent text-secondary-50 rounded-md flex items-center gap-1 text-sm"
          >
            View all
            <PiCaretRight className="" />
          </Link>
        </div>
      </div>
      {offers.length > 0 ? (
        <ul
          className={`grid ${
            cols === 1
              ...
              ? "grid-cols-2"
              : "grid-cols-3"
          } py-2 gap-3 sm:gap-4 w-full`}
        >
          {offers.slice(0, maxShowOffers).map((offer) => {
                ...
                <div className="card-body p-4 sm:p-6 justify-between space-y-3 sm:space-y-4 relative rounded-lg">
                  ...
                  <div className="flex justify-between items-start gap-2">
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <NameUsers
                          user={offer.owner}
                          classProps="text-xs sm:text-sm truncate"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <h3 className="text-lg sm:text-xl font-bold line-clamp-2">
                      {offer.position || offer.title}
                    </h3>
                    <p className="text-neutral-10 text-sm sm:text-base">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                    ...
                    <div className="badge text-neutral-0 bg-neutral-60 text-xs w-fit">
                      {offer.contractType}
                    </div>
                  </div>

                  <div>
                    <p className="line-clamp-2 sm:line-clamp-3 text-sm sm:text-base">
                      {offer.description}
                    </p>
                  </div>
                  ...
                  <Link
                    to={`/offers/${offer._id}`}
                    className="justify-self-center btn bg-gradient-to-r from-secondary-50 to-primary-50 rounded-md flex items-center gap-1"
                  >
                    View Offer
                  </Link>
                </div>
